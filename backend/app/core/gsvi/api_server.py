...
def init_default_models():
    ...
    try:
        ...
        if not sovits_path or not gpt_path:
            logger.warning("配置文件中未找到默认模型路径")
            return False
        
        # 检查文件是否存在
        if not os.path.exists(sovits_path):
            logger.error(f"默认 SoVITS 模型文件不存在: {sovits_path}")
            return False
            
        if not os.path.exists(gpt_path):
            logger.error(f"默认 GPT 模型文件不存在: {gpt_path}")
            return False
        
        logger.info("开始加载默认模型...")
        
        # 加载 SoVITS 模型
        logger.info(f"加载 SoVITS 模型: {sovits_path}")
        change_sovits_weights(sovits_path)
        current_sovits_path = sovits_path
        
        # 加载 GPT 模型
        logger.info(f"加载 GPT 模型: {gpt_path}")
...
async def startup_event():
    ...
    success = init_default_models()
    if success:
        logger.info("服务器启动完成，默认模型已加载")
    else:
        logger.warning("服务器启动完成，但默认模型加载失败，请手动切换模型")

@app.post("/models/switch", summary="切换模型")
...
@app.get("/models/status", summary="获取模型状态")
...
@app.post("/tts/base64", summary="语音合成 (Base64)")
...
@app.get("/characters", summary="获取角色和情绪列表")
